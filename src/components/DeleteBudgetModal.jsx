...
const DeleteBudgetModal = ({ isOpen, onClose, onConfirm, budgetName }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md m-4 relative transform transition-all" onClick={e => e.stopPropagation()}>
        ...
        <div className="text-center mt-4">
            <h2 className="text-xl font-bold text-slate-800">Eliminar Presupuesto</h2>
            <p className="text-slate-500 mt-2">
                ¿Estás seguro de que quieres eliminar el presupuesto para <strong className="text-slate-700">{budgetName}</strong>? Esta acción no se puede deshacer.
            </p>
        </div>
