  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <input
        ...
        className="w-20 text-center px-2 py-1 bg-slate-50 border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500"
        placeholder="0"
      />
      <span className="w-24 text-right font-semibold text-slate-800">{formatCurrency(subtotal)}</span>
    </div>
  );
};
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-2xl m-4 relative transform transition-all" onClick={e => e.stopPropagation()}>
        ...
        <h2 className="text-2xl font-bold text-slate-800 mb-2">Contador de Efectivo</h2>
        <p className="text-slate-500 mb-6">Ingresa la cantidad de cada billete y moneda.</p>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 max-h-[50vh] overflow-y-auto pr-4">
          {/* Columna de Billetes */}
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-slate-700 text-center border-b pb-2">Billetes</h3>
            {billetes.map(item => (
              <DenominationRow 
                key={item.value}
                item={item}
          ...
          {/* Columna de Monedas */}
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-slate-700 text-center border-b pb-2">Monedas</h3>
            {monedas.map(item => (
              <DenominationRow 
                key={item.value}
                item={item}
        ...
        <div className="mt-6 border-t border-slate-200 pt-4">
          <div className="flex justify-between items-center">
            <span className="text-lg font-bold text-slate-800">Total Contado:</span>
